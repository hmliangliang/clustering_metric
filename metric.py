...
def metric(y, y_pre):
    ...
    R = (TP + TN)/N #Rand系数
    J = TP/(TP+FN+FP) #Jaccard系数
    ARI = 2*(TP*TN-FN*FP)/((TP+FN)*(FN+TN)+(TP+FP)*(FP+TN)) #ARI指数
    FM = TP/(math.sqrt((TP+FN)*(TP+FP)))#Folkes and Mallows指标
    prcision = TP/(TP+FP)
    recall = TP/(TP+FN)
    F1 = 2*prcision*recall/(prcision+recall)#计算F-measure指标
    Hubert = (N*TP-(TP+FN)*(TP+FP))/math.sqrt((TP+FN)*(TP+FP)*(TN+FN)*(TN+FP))#Hubrt指数，范围[-1,1]，其值越大越好
    Phi = (TP*TN-FN*FP)/math.sqrt((TP+FN)*(TP+FP)*(FN+TN)*(FP+TN))#Phi指数，范围[-1,1]，其值越大越好
    K = 1/2*(TP/(TP+FP)+TP/(TP+FN))#Kulczynski指标
    RT = (TP+TN)/(TP+TN+2*(FN+FP))#Rogers-Tanimoto指标
